...
if isfolder("neverpeed") == false then
    makefolder("neverpeed")
end

if isfolder("neverpeed/sound") == false then
    makefolder("neverpeed/sound")
end

if isfolder("neverpeed/sound/mc") == false then
    makefolder("neverpeed/sound/mc")
end

if isfolder("neverpeed/assets") == false then
    makefolder("neverpeed/assets")
end

if isfolder("neverpeed/config") == false then
    makefolder("neverpeed/config")
end

if isfolder("neverpeed/scripts") == false then
    makefolder("neverpeed/scripts")
end

local sliderapi = {}

local foldername = "neverpeed/config"
local conf = {
...
ScreenGuitwo.Name = "neverpeedNotificationGui"
...
local function getcustomassetfunc(path)
    if not betterisfile(path) then
        ...
        local req = requestfunc({
            Url = "https://raw.githubusercontent.com/7GrandDadPGN/VapeV4ForRoblox/main/"..path:gsub("neverpeed/assets", "assets"),
            Method = "GET"
        })
...
local function getcustomassetfuncforsounds(path)
    if not betterisfile(path) then
        ...
        local req = requestfunc({
            Url = "https://raw.githubusercontent.com/8pmX8/rektsky4roblox/main/"..path:gsub("neverpeed/sound", "sound"),
            Method = "GET"
        })
        writefile(path, req.Body)
    end
...
local function playdingsound(istrue) 
    if lib["Sounds"] then
        if istrue then
            playsound(getcustomassetfuncforsounds("neverpeed/sound/enable.mp3") or getasset("neverpeed/sound/enable.mp3"))
        else
            playsound(getcustomassetfuncforsounds("neverpeed/sound/disable.mp3") or getasset("neverpeed/sound/disable.mp3"))
        end
    end
end
...
function lib:CreateWindow()
    ...
    function lib:CreateTab(title, color, asset)
        ...
        assetthing.Image = getcustomassetfunc("neverpeed/assets/" .. asset .. ".png") or getasset("neverpeed/assets/" .. asset .. ".png")
        local tabtable = {
            ["Toggles"]={}
        }
